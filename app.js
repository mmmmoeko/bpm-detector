(() => {
  ...
  function autocorrelationBPM(odf, sampleRate, numFrames) {
    ...
    // Convert to BPM scores with parabolic interpolation for sub-lag precision
    const scores = new Map();
    for (let lag = lagMin + 1; lag <= lagMax - 1; lag++) {
      if (enhanced[lag] <= 0) continue;
      // Only process local peaks
      if (enhanced[lag] < enhanced[lag - 1] || enhanced[lag] < enhanced[lag + 1]) {
        // Still add non-peak lags at lower weight for coverage
        const bpm = Math.round((framesPerSec * 60 / lag) * 10) / 10;
        if (bpm >= BPM_MIN && bpm <= BPM_MAX) {
          const prev = scores.get(bpm) || 0;
          scores.set(bpm, Math.max(prev, enhanced[lag] * 0.8));
        }
        continue;
      }
      // Parabolic interpolation around the peak
      const y0 = enhanced[lag - 1];
      const y1 = enhanced[lag];
      const y2 = enhanced[lag + 1];
      ...
      const bpm = Math.round((framesPerSec * 60 / refinedLag) * 10) / 10;
      if (bpm >= BPM_MIN && bpm <= BPM_MAX) {
        const prev = scores.get(bpm) || 0;
        scores.set(bpm, Math.max(prev, refinedVal));
      }
    }
  ...
  function fusedBestBPM(acScores, combScores) {
    ...
    // Merge into unified BPM grid (round to 0.1)
    const fused = new Map();
    const allBPMs = new Set();
    for (const bpm of acNorm.keys()) allBPMs.add(Math.round(bpm * 10) / 10);
    for (const bpm of combNorm.keys()) allBPMs.add(Math.round(bpm * 10) / 10);

    for (const bpm of allBPMs) {
      const ac = findNearestScore(acNorm, bpm, 0.3);
      const comb = findNearestScore(combNorm, bpm, 0.3);
  ...
  function displayResult(data) {
    ...
    resultFileName.textContent = fileName.textContent;
  ...
  function drawWaveform(mono) {
    ...
    gradient.addColorStop(0, "#3cbfd9");
    gradient.addColorStop(0.5, "#2aadc5");
    gradient.addColorStop(1, "#1a9ab2");
